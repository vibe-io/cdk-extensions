 ...
 *
 * This is implemented outside of the FlowLogField class to ensure it is
 * available when building static properties while also making linting happy.
 */
const registeredFields: {[name: string]: FlowLogField} = {};
...
export class FlowLogField {
   ...
   * [available fields](https://docs.aws.amazon.com/vpc/latest/userguide/flow-logs.html#flow-logs-fields).
   */
  public constructor(name: string, type: FlowLogDataType) {
    this.name = name;
    this.type = type;

    let conflict = false;
    if (name in registeredFields) {
      const existing = registeredFields[name];
